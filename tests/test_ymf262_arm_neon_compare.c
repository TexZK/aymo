...
static int compare_slots(int slot_)
{
    ...
    int16_t channel_fb = (slot->channel->fb ? -(int16_t)(9u - slot->channel->fb) : +16);
    assert((int16_t)vextractn(sg->wg_fb_shs, sgo) == channel_fb);
#ifdef AYMO_DEBUG
...
static int compare_chips(void)
{
    ...
    assert(vget_lane_s16(aymo_chip.og_out, 0) == nuked_out[0]);
    assert(vget_lane_s16(aymo_chip.og_out, 1) == nuked_out[1]);
    assert(vget_lane_s16(aymo_chip.og_out, 2) == nuked_out[2]);
    assert(vget_lane_s16(aymo_chip.og_out, 3) == nuked_out[3]);
