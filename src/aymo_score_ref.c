...
)
{
    ...
    score->offset = 0u;
    aymo_score_ref_skip_whitespace(score);
    if ((score->offset < score->size) && (score->text[score->offset] == 'i')) {  //  "init\n"
...
)
{
    ...
    if AYMO_UNLIKELY(c == 'i') {  // "init\n"
        aymo_score_ref_skip_line(score);
    }
    else if AYMO_UNLIKELY(c == 's') {  // "setchip(%d)\n"
